<form autocomplete="off" class="appForm clearfix" method="post" enctype="application/x-www-form-urlencoded">
    <fieldset>
        ...
        <div class="input_wrapper n100">
            ...
            <input required type="text" name="GroupName" id="GroupName" maxlength="20" value="<?= $Usersgroup->GroupName ?>">
        </div>
        <div class="input_wrapper_other">
            <label><?= $text_label_privileges ?></label>
            <?php if ($privilig !== false): foreach ($privilig as $privilig): ?>
                <label class="checkbox block">
                    <input type="checkbox" name="privileges[]" id="privileges" <?= in_array($privilig->PrivilegeId , $usersgrouppriviligid) ? 'checked' : '' ?> value="<?= $privilig->PrivilegeId ?>">
